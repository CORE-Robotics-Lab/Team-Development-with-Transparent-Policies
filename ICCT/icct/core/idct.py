...
class IDCT(nn.Module):
    def __init__(self,
        ...
        """
        Initialize the Interpretable Discrete Control Tree (IDCT)
        ...
        :param leaves: the number of leaves of IDCT
        ...
        :param device: which device should IDCT run on [cpu|cuda]
    ...
    def init_submodels(self):
        self.lin_models = nn.ModuleList(
                [nn.Linear(self.input_dim, self.output_dim) for _ in range(self.num_leaves)])
    ...
    def fs_submodels(self, input):
        ...
        if self.fs_submodel_version == 1:
            ...
            new_i = new_w.unsqueeze(0).expand(batch_size, num_leaves, output_dim, input_dim) * input.unsqueeze(
                1).unsqueeze(2).expand(batch_size, num_leaves, output_dim, input_dim)
    ...
    def forward(self, input_data, embedding_list=None):
        ...
        # mus = probs @ self.action_probs
        if not self.is_value:
            return self.softmax(mus)
        else:
            return mus
