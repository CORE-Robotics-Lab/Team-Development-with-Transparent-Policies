# Created by Yaru Niu
# Revised from https://github.com/DLR-RM/stable-baselines3/blob/master/stable_baselines3/sac/sac.py
...
from ICCT.icct.rl_helpers.sac_discrete_policies import SACDiscretePolicy
from ICCT.icct.rl_helpers.ddt_sac_discrete_policy import DDT_SACDiscretePolicy
...
class SACDiscrete(OffPolicyAlgorithm):
    ...
    def __init__(
        self,
        policy: Union[str, Type[SACDiscretePolicy]],
    ...
    def _setup_model(self) -> None:
        ...
        if isinstance(self.ent_coef, str) and self.ent_coef.startswith("auto"):
            ...
            self.log_ent_coef = torch.Tensor([np.log(init_value)]).to(self.device)
            self.log_ent_coef.requires_grad = True
            self.ent_coef_tensor = self.log_ent_coef.exp()
...
#         self._update_learning_rate(optimizers)
        ...
        for gradient_step in range(gradient_steps):
            ...
            with torch.no_grad():
                ...
                next_act_probs, next_act_log_probs = self.actor.action_info(replay_data.next_observations)
            ...
            current_q_values1 = current_q_values1.gather(1, replay_data.actions).reshape(-1)
            current_q_values2 = current_q_values2.gather(1, replay_data.actions).reshape(-1)
            critic_loss = 0.5 * sum([F.mse_loss(current_q, target_q_values) for current_q in [current_q_values1, current_q_values2]])
            ...
            # https://github.com/p-christ/Deep-Reinforcement-Learning-Algorithms-with-
            # PyTorch/blob/135d3e2e06bbde2868047d738e3fc2d73fd8cc93/agents/actor_critic_agents/SAC_Discrete.py#L89
            # act_log_probs = torch.sum(act_log_probs * act_probs, dim=1)
            ...
            if self.ent_coef_optimizer is not None:
                ...
                ent_coef_loss = -(self.log_ent_coef * (act_log_probs + self.target_entropy).detach()).mean()
                ent_coef_losses.append(ent_coef_loss.item())

            ent_coefs.append(self.log_ent_coef.detach().exp().item())
