...
from ICCT.icct.core.icct_helpers import convert_to_crisp, convert_to_crisp_idct
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='IDCT Testing')
    parser.add_argument('--env_name', help='environment to run on', type=str, default='cartpole')
    ...
    model = PPO.load("../../" + args.load_path + "/" + args.load_file, device=args.device)
    obs = env.reset()
    ...
    env, env_n = make_env(args.env_name, args.seed)
    if hasattr(model.policy, 'ddt'):
        model.policy.ddt = convert_to_crisp_idct(model.policy.ddt, training_data=None)
        obs = env.reset()
    ...
    from icct.interactive.visualize import IDCTVisualizer

    icct = model.policy.ddt
    visualizer = IDCTVisualizer(icct, args.env_name)
    # visualizer.modifiable_gui()
    visualizer.export_gui('cartpole_tree.png')