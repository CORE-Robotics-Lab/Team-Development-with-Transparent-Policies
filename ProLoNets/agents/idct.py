...
class IDCT(nn.Module):
    ...
    def init_comparators(self, comparators):
        if self.force_rand_init or comparators is None:
            comparators = []
    ...
    def init_weights(self, weights):
        if self.force_rand_init or weights is None:
            weights = []
            if type(self.leaf_init_information) is int:
                depth = int(np.floor(np.log2(self.leaf_init_information)))
            else:
    ...
    def init_selector(self, selector, weights):
        if self.force_rand_init or selector is None:
            if weights is None:
    ...
    def diff_argmax(self, logits, dim=-1):
        tau = 1.0
        sample = False # self.use_gumbel_softmax
    ...
    def forward(self, input_data, embedding_list=None):
        ...
        if self.hard_node:
            ...
            comps = self.comparators / divisors
            weights = self.layers * onehot_weights / divisors
            alpha = self.alpha

        input_data = input_data.t().expand(self.layers.size(0), *input_data.t().size())

        input_data = input_data.permute(2, 0, 1)
        comp = weights.mul(input_data)
        if not self.vectorized:
            comp = comp.sum(dim=2).unsqueeze(-1)
        comp = comp.sub(comps.expand(input_data.size(0), * comps.size()))
        comp = comp.mul(alpha)
