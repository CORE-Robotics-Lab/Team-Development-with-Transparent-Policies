...
from ProLoNets.agents.idct_helpers import init_cart_nets, swap_in_node, add_level, \
    init_lander_nets, init_adversarial_net, save_prolonet, load_prolonet
import copy
import os
...
class IDCT_Agent:
    ...
    def duplicate(self):
        new_agent = IDCT_Agent(distribution='one_hot',
                               bot_name=self.bot_name,
                               input_dim=self.input_dim,
                               output_dim=self.output_dim,
                               use_gpu=self.use_gpu,
