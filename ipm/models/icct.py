...
class ICCT(nn.Module):
    ...
    def init_submodels(self):
        if self.sparse_submodel_type != 2:
            ...
            self.sub_scalars = nn.Parameter(
                torch.zeros(self.num_leaves, self.output_dim, self.input_dim).to(self.device), requires_grad=True)
            self.sub_weights = nn.Parameter(
                torch.zeros(self.num_leaves, self.output_dim, self.input_dim).to(self.device), requires_grad=True)
            self.sub_biases = nn.Parameter(
                torch.zeros(self.num_leaves, self.output_dim, self.input_dim).to(self.device), requires_grad=True)
